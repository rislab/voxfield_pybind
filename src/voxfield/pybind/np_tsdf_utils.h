...
bool projectPointCloudToImage(
    ...
    cv::Mat& color_image,
    float min_z, 
    float min_d, const NpTsdfServerConfig& config) const {
...
float projectPointToImageLiDAR(
    const Point& p_C, int* u, int* v, const NpTsdfServerConfig& config) const {
...
bool projectPointToImageCamera(
    const Point& p_C, int* u, int* v, const NpTsdfServerConfig& config) const {
  CHECK_NOTNULL(u);
  *u = std::round(p_C.x() * fx_ / p_C.z() + config.vx_);
  if (*u >= config.width_ || *u < 0) {
    return false;
  }
  CHECK_NOTNULL(v);
  *v = std::round(p_C.y() * fy_ / p_C.z() + config.vy_);
...
cv::Mat computeNormalImage(
    const cv::Mat& vertex_map, const cv::Mat& depth_image, const NpTsdfServerConfig& config) const {
